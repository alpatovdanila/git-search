...
import {SortOption, sortOptions} from "@/features/search/components/sort-select/options";
...
const invertOrder = (order:SearchOrder): SearchOrder => order === 'desc' ? 'asc' : 'desc';

const orderArrow = (order:SearchOrder) => (order === 'desc') ? '🠗' : '↑';

export const SortSelect = ({sort='best-match', onSortChange, order = 'desc', onOrderChange}: Props) => {

    const handleSortSelect = (sortOption: SortOption) => {
        if (!!onOrderChange && sort === sortOption.sort) onOrderChange(invertOrder(order)); else {
